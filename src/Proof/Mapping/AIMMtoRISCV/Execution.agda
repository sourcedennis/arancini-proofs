...
module Proof.Mapping.AIMMtoRISCV.Execution
...
open RISCV-AIMMRestricted dst-ok
...
ev[⇐] : {x : EventRISCV} → (x∈dst : x ∈ events dst) → EventAIMM
...
module Extra where
  ...
  ev-rwfs : (x : EventAIMM) → (EvR ∪₁ EvW ∪₁ EvF ∪₁ EvSkip) x
  ...
  Fₜ[$⇒] : {t : AIMM.LabF} → Pred[$⇒] (EvFₜ t) (EvFₜ (rule-labf t))
