...
open import Arch.TCG.Detour src

open TCG.Relations
open Armv8-TCGRestricted dst-ok
open Armv8.Relations dst
open IsArmv8Consistent

open TCG.Properties src-wf
...
src-ax-coherence refl coh[xx] =
  ...
  in ax-coherence dst-consistent refl (⁺[⇒]ˡ cohˡ∈ex coh[⇒] x∈src x∈src coh[xx])
  where
  coh-dom : ∀ {x y : EventTCG} → Coh src x y → x ∈ events src
  ...
  coh[⇒] : Rel[⇒] (Coh src) (po-loc dst ∪₂ Ca dst-a8 ∪₂ rf dst)
  coh[⇒] x∈src y∈src (coh-po-loc po-loc[xy]) = opt₁ (po-loc[⇒] x∈src y∈src po-loc[xy])
  coh[⇒] x∈src y∈src (coh-rf rf[xy])         = opf₃ (rf[⇒] x∈src y∈src rf[xy])
  coh[⇒] x∈src y∈src (coh-fr fr[xy])         = opt₂ (ca-fr (fr[⇒] x∈src y∈src fr[xy]))
  coh[⇒] x∈src y∈src (coh-co co[xy])         = opt₂ (ca-co (co[⇒] x∈src y∈src co[xy]))
...
RW⨾po⨾Rₜ[⇒]bob : {x y : EventTCG} (x∈src : x ∈ events src) (y∈src : y ∈ events src)
  → po src x y → EvRW x → EvRₜ trmw y
  → ( po dst ⨾ ⦗ EvFₘ F-full ⦘ ⨾ po dst ) (ev[⇒] x∈src) (ev[⇒] y∈src)
RW⨾po⨾Rₜ[⇒]bob x∈src y∈src po[xy] x-rw y-rₜ =
  ...
  λ{ (inj₁ refl)   → ⊥-elim (disjoint-f/rw _ (fₜ⇒f v-sc , RW[⇒] x∈src x-rw)) -- x ≡ v
   ; (inj₂ po[xv]) → po[xv] ⨾[ _ ]⨾ (refl , v-sc) ⨾[ _ ]⨾ proj₁ pi[vy]
   }

Wₜ⨾po⨾RW[⇒]bob : {x y : EventTCG} (x∈src : x ∈ events src) (y∈src : y ∈ events src)
...
Wₜ⨾po⨾RW[⇒]bob x∈src y∈src po[xy] x-wₜ y-rw =
  ...
  λ{ (inj₁ refl)   → ⊥-elim (disjoint-f/rw _ (fₜ⇒f z-sc , RW[⇒] y∈src y-rw)) -- y ≡ z
...
src-ax-global-ord refl ghb[xx] =
  ...
  ... | no x∉rmwˡ = -- failed RMW
          ...
          λ{ (inj₁ refl) → ⊥-elim (disjoint-f/rw _ (fₜ⇒f v-sc , RW[⇒] y∈src y-rw)) -- v ≡ y
           ; (inj₂ po[vy]) →
               obi-lob [ lobi-bob (bob-f (proj₁ pi[xv] ⨾[ _ ]⨾ (refl , v-sc) ⨾[ _ ]⨾ po[vy])) ]
           }

  ghbi[⇒]obi : Rel[⇒] GhbiAlt (Obi dst-a8)
