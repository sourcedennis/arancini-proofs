...
-- | The RISC-V memory model. Taken from the RISC-V spec (V20191213 - section 14.1).
--
-- Note that it does *not* support mixed-size accesses. Hence, we ignore those.
...
record RISCVExecution (ex : Execution {arch-RISCV}) : Set₁ where
  open Execution ex
  field
...
module Relations
  {ex : Execution {arch-RISCV}}
  (rex : RISCVExecution ex) where
  ...
  -- relates any two read events that read from the same write event
  rsw : Rel₀ Event
  rsw = flip rf ⨾ rf
  ...
  data Ppo (x y : Event) : Set where
    -- this is an extra rule we require for initialization events, which
    -- represent the memory at the start. they happen, by definition, before
    -- all other events.
    ppo₀  : ( ⦗ EvInit ⦘ ⨾ po )  x y → Ppo x y
    ...
    --
    -- Minor issue here: The .cat model considers `rfi` to be `rf ∩ (po ∪ po⁻¹)`.
    -- Yet, `po⁻¹` is not really a *preserved program order*. Regardless, we can
    -- use this definition, as `poloc;rf` is disallowed by coherence anyway.
    ppo₃  : ( ⦗ EvWₐ ∪₁ EvSc ⦘ ⨾ rfi ⨾ ⦗ EvR ⦘ ) x y → Ppo x y
    -- # Explicit Synchronization
    ppo₄  : Fob                                              x y → Ppo x y
    ppo₅  : ( ⦗ EvAcq ⦘ ⨾ po ⨾ ⦗ EvRW ⦘ )                    x y → Ppo x y
    ppo₆  : ( ⦗ EvRW ⦘ ⨾ po ⨾ ⦗ EvRel ⦘ )                    x y → Ppo x y
    -- > x and y both have RCsc annotations
    ...
    -- Similar minor issue as with `ppo₃`
  ...
  record IsRISCVConsistent : Set where
    field
      ...
      ax-model     : Acyclic _≡_ ( co ∪₂ rfe ∪₂ fr ∪₂ Ppo ) -- external visibility
